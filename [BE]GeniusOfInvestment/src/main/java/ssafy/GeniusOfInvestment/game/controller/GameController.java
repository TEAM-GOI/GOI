...
public class GameController {
    ...
    @GetMapping("/start")
    public SuccessResponse<Void> getInitStockInfo(@AuthenticationPrincipal User user, @RequestParam("id") Long grId){
...
//        redisTemplate.opsForValue().set("refresh" + grId, str); //새로고침에 대한 기능을 막기 위해서
        return SuccessResponse.from(SuccessType.GAME_INIT_INFO_COMPLETE);
    }

    //게임 시작시 페이지를 넘기기 위한 API
    @GetMapping("")
    public SuccessResponse<Void> startGame(@AuthenticationPrincipal User user, @RequestParam("id") Long grId){
        gameService.startGame(user, grId);
        Map<String, String> json = new HashMap<>();
        json.put("msg", "게임 시작 완료");
        sendMsg(grId, json, MessageDto.MessageType.START);
        return SuccessResponse.from(SuccessType.GAME_START);
    }

    //다음 턴 넘기기(참가한 모든 유저에 대한 평가 금액도 모두 함께 계산)
    @GetMapping("/next")
    public SuccessResponse<Void> getNextStockInfo(@AuthenticationPrincipal User user, @RequestParam("id") Long grId){
//        redisTemplate.opsForValue().set("thread" + grId, "STOP");
        ...
        return SuccessResponse.from(SuccessType.NEXT_TURN);
    }

    //유저가 레디를 눌렀다.
    @PutMapping("/ready/{id}")
    public SuccessResponse<Void> doingReady(@AuthenticationPrincipal User user, @PathVariable("id") Long grId){
        ...
        if(!rsp.start()){ //아직 전체 참여자가 레디를 다 누르지 않았다.
            ...
            return SuccessResponse.from(SuccessType.READY);
...
//                log.info("새로운 타이머 생성 후");
    ...
    //게임 정상 종료
    @PutMapping("/end/{id}")
    public SuccessResponse<Void> endGame(@PathVariable("id") Long grId){
        List<ParticipantInfo> rst = gameService.endGame(grId);
        sendMsg(grId, rst, MessageDto.MessageType.GAME_RESULT);
        return SuccessResponse.from(SuccessType.END_GAME);
    }

    //유저 게임방 탈퇴
    @DeleteMapping("/exit/{id}")
    public SuccessResponse<Void> exitGame(@AuthenticationPrincipal User user, @PathVariable("id") Long grId){
        ...
        if(num.size() == 1){ //1명이 남았을 경우에는 게임을 종료
            ...
            return SuccessResponse.from(SuccessType.EXIT_GAME);
        }
    }
